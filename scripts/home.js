...
function rimuoviPreferito(event){
    ...
    preferiti[i].classList.add('preferiti');
            ...
            formData.append('titolo',titolo.textContent);
        ...
        function insertDB(event){
            for(let i=0;i<6;i++){
                if(preferiti[i]===event.target){
                    ...
                    fetch("http://localhost/hw1/preferiti_utenti.php",{
                    method:"post",
                  body:formData
...
fetch("http://localhost/hw1/contenuti_home.php").then(onResponse).then(onJson);
...
fetch("http://localhost/hw1/controllo_preferiti.php").then(onResponse).then(onJson3);
